...
#+begin_src emacs-lisp :results raw drawer replace :exports results 
(concat
 (mapconcat
  (lambda (i)
    (let ((xx (format "%05d" i)))
      (format "** Iterative policy evaluation\n[[file:figures/eval_%s_values.svg]] [[file:figures/eval_%s_policy.svg]] \n\n- $k = %s$\n"
              xx xx xx)))
   '(0 1 2 3 10 10000)
...
** code                                                            :noexport:
#+begin_src emacs-lisp :results raw drawer :exports results 
(concat
 (mapconcat
  (lambda (i)
    (let ((xx (format "%02d" i)))
      (format "** Iterative policy improvement\n[[file:figures/iter_%s_values.svg]] [[file:figures/iter_%s_policy.svg]] \n\n- $k = %s$\n"
              xx xx xx)))
  (number-sequence 0 10)
  "\n"))
#+end_src
