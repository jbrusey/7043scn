...
** DP: Edit distance between kitten and sitting
#+attr_html: :height 400px
[[file:figures/kitten1.svg]]

- Since "k" is not equal to "s", we take the minimum of the two table values above and to the left. 
** DP: Edit distance between kitten and sitting
#+attr_html: :height 400px
[[file:figures/kitten2.svg]]
- Since both prefixes have "i" here, we can copy, meaning we take the value from the diagonal above left
** DP: Edit distance between kitten and sitting
#+attr_html: :height 400px
[[file:figures/kitten3.svg]]
- The whole path to edit the string (from kitten to sitting) can be seen as +k+ *s* itt +e+ *i* n *g*
- By the way, the same approach can be used for whole files by representing each line with its hash
...
[[file:figures/recycle-robot.png]]
...
[[file:figures/pol-eval-alg.png]]

** code for iterative policy eval                                  :noexport:
#+begin_src emacs-lisp :results raw replace :exports results 
(concat
 (mapconcat
  (lambda (i)
    (let* ((xx (format "%05d" i))
           (header "** Iterative policy evaluation\n")
           (left (format
                  "#+REVEAL_HTML: <div style=\"display: flex;\"><div style=\"flex: 1; padding: 0px;\">\n\
Value $V^\\pi_k$ of random policy at step $k$\n\n\
[[file:figures/eval_%s_values.svg]]\n\n\
$k = %d$\n\
#+REVEAL_HTML: </div><div style=\"flex: 1; padding: 0px; \">\n"
                  xx i))
           (right (format
                   "Greedy policy based on this value\n\n\
[[file:figures/eval_%s_policy.svg]]\n\
#+REVEAL_HTML: </div></div>"
                   xx)))
      (concat header left right)))
  '(0 1 2 3 10 10000)
  "\n")))
#+end_src
...
[[file:figures/pol-iteration-alg.png]]
